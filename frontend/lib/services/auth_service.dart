...
class AuthService {
  ...
  static String get _baseUrl {
    if (kIsWeb) return 'http://127.0.0.1:8080/api/auth';
    if (Platform.isAndroid) return 'http://10.0.2.2:8080/api/auth';
  ...
  Future<LoginResponse> login(String email, String password) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          "Statut de connexion inattendu: ${response.statusCode}",
        );
      }
      ...
      if (e.response?.statusCode == 401) {
        ...
        throw Exception(
          "Erreur réseau: Impossible de joindre le serveur. Veuillez réessayer plus tard.",
        );
      }
    } catch (e) {
      if (kDebugMode) print('❌ [AUTH] Erreur inattendue: $e');
      rethrow;
    }
  }
