...
class ApiClient {
  ...
  static String get baseUrl {
    if (kIsWeb) {
      ...
      return 'http://192.168.68.119:8080/api/v1';
    }
  }
  ...
  static Future<void> init() async {
    ...
    _dio!.interceptors.add(
      // Utiliser _dio! car il vient d'être initialisé
      InterceptorsWrapper(
        onRequest: (options, handler) async {
          ...
          if (token != null && token.isNotEmpty) {
            ...
            if (kDebugMode) {
              ...
              print(
                '⚠️ [API] Aucun token trouvé dans Hive pour ${options.path}',
              );
            }
          }
        ...
        onError: (DioException e, handler) async {
          ...
          if (e.response?.statusCode == 403 || e.response?.statusCode == 401) {
            ...
            if (kDebugMode) {
              print(
                '🚨 [DEBUG] Requête renvoyée 401/403. Déconnexion automatique désactivée pour debug.',
              );
              print(
                '🚨 [DEBUG] Vérifiez le token manuellement et testez avec curl.',
              );
            }
  ...
  static Dio get dio {
    if (_dio == null) {
      throw Exception(
        'Dio client n\'a pas été initialisé. Appelez ApiClient.init() au démarrage.',
      );
    }
    return _dio!;
  }
