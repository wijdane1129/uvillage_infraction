    ...
    public ResponseEntity<?> login(@Valid @RequestBody LoginRequest request) {
        ...
        try {
            Authentication authentication = authenticationManager.authenticate(
                ...
                // Generate token using the user's email (JwtUtils exposes generateToken(String))
        ...
        if (principal != null) {
            ...
            try {
                ...
                if (jwtUtils.validateToken(token)) {
                    // JwtUtils stores the subject as the email when generating tokens
                    email = jwtUtils.getUserIdFromToken(token);
                }
            } catch (Exception ex) {
